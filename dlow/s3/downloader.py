...
class S3FolderDownloader(ResourceDownloader):
    ...
    def __init__(self, s3_bucket_name, s3_top_level_source_folder, logger=logging.getLogger()):
    ...
    def _download_s3_dir(self, dest_dir, recursive=True, s3_current_source_folder=None):
        ...
        for s3_objects_list in paginator.paginate(Bucket=self._s3_bucket_name, Delimiter='/', Prefix=s3_current_source_folder):
            if recursive:
                ...
                if s3_folders is not None:
                    for subfolder in s3_folders:
                        ...
                        for downloaded_file_path in self._download_s3_dir(dest_dir, recursive=True, s3_current_source_folder=subfolder.get('Prefix')):
            ...
            for s3_file_object in s3_file_objects:
                ...
                if not os.path.exists(dest_folder_of_file):
                    self._logger.info('Creating local directory %s' % (dest_folder_of_file,))
                    os.makedirs(dest_folder_of_file)
                self._logger.info('Starting download of object %s from bucket %s to local directory %s' % (
                ...
                self._logger.info('Finished download of object %s from bucket %s to local directory %s' % (s3_object_key, self._s3_bucket_name, dest_path_of_file))

                yield dest_path_of_file

    def iter_downloaded_files(self, dest_dir, recursive=True):
        for downloaded_file_path in self._download_s3_dir(dest_dir, recursive):
            yield downloaded_file_path